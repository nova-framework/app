...
Route::group(array('prefix' => 'admin', 'namespace' => 'App\Modules\Users\Controllers\Admin'), function() {
    ...
    Route::get( 'users/(:num)',         array('before' => 'auth',      'uses' => 'Users@show'));
    Route::get( 'users/(:num)/edit',    array('before' => 'auth',      'uses' => 'Users@edit'));
    Route::post('users/(:num)',         array('before' => 'auth|csrf', 'uses' => 'Users@update'));
    Route::post('users/(:num)/destroy', array('before' => 'auth|csrf', 'uses' => 'Users@destroy'));
    ...
    Route::get( 'roles/(:num)',         array('before' => 'auth',      'uses' => 'Roles@show'));
    Route::get( 'roles/(:num)/edit',    array('before' => 'auth',      'uses' => 'Roles@edit'));
    Route::post('roles/(:num)',         array('before' => 'auth|csrf', 'uses' => 'Roles@update'));
    Route::post('roles/(:num)/destroy', array('before' => 'auth|csrf', 'uses' => 'Roles@destroy'));
});
